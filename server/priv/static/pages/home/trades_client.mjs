...
function enctype(value3) {
  return attribute("enctype", value3);
}
...
// build/dev/javascript/trades_client/trades_client.mjs
var State = class extends CustomType {
  constructor(files, count2) {
    super();
    this.files = files;
    this.count = count2;
  }
};
var FileSelectionChanged = class extends CustomType {
  constructor(files) {
    super();
    this.files = files;
  }
};
var IncrementedCount = class extends CustomType {
  constructor(count2) {
    super();
    this.count = count2;
  }
...
function init2(_) {
  return [new State(toList([]), 0), none()];
}
function update(model, msg) {
  let new_state = (() => {
    if (msg instanceof FileSelectionChanged) {
      let files = msg.files;
      debug(files);
      return new State(files, count(files, (_) => {
        return true;
      }));
    } else {
      let count2 = msg.count;
      return model.withFields({ count: count2 });
    }
  })();
  return [new_state, none()];
}
function extract_from_file(file) {
  let field2 = field("name", string);
  let $ = field2(file);
  if ($.isOk()) {
    let str = $[0];
    return str;
...
function retrieve_file_name_from_file_obj(loop$index, loop$file_list, loop$accumulator) {
  while (true) {
    let index2 = loop$index;
    let file_list = loop$file_list;
    let accumulator = loop$accumulator;
    let field2 = field(to_string(index2), dynamic);
    let $ = field2(file_list);
    if ($.isOk()) {
      let file_obj = $[0];
      let new_accumulator = prepend(
        extract_from_file(file_obj),
        accumulator
      );
      loop$index = index2 + 1;
      loop$file_list = file_list;
      loop$accumulator = new_accumulator;
    } else {
      return accumulator;
    }
  }
}
function on_change(event2) {
  return try$(
    field("target", dynamic)(event2),
    (target) => {
      return try$(
        field("files", dynamic)(target),
        (file_list) => {
          let file_names = retrieve_file_name_from_file_obj(
            0,
            file_list,
            toList([])
          );
          return new Ok(new FileSelectionChanged(file_names));
        }
      );
    }
  );
}
function file_table(files) {
  return table(
    toList([]),
    map(
      files,
      (file) => {
        return tr(
          toList([]),
          toList([
            td(toList([]), toList([text2(file)])),
            td(
              toList([]),
              toList([button(toList([]), toList([text2("D")]))])
            )
          ])
        );
      }
...
function increment_count(event2) {
  return try$(
    value2(event2),
    (value3) => {
      ...
      if ($.isOk()) {
        let existing = $[0];
        return new Ok(new IncrementedCount(existing + 1));
      } else {
        return new Error(
          toList([new DecodeError("a number", value3, toList([]))])
        );
      }
    }
  );
...
function view(model) {
  return div(
    ...
    toList([
      ...
      input(
        toList([
          value(to_string(model.count)),
          on("change", increment_count)
        ])
      ),
      input(
        toList([
